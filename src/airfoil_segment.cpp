...
std::tuple<glm::dvec3, glm::dvec3> AirfoilSegment::getForceAndTorque(
    ...
    glm::dvec3 segmentVel = vel + glm::cross(angVel, pos);
    ...
    debugArrows.push_back(std::make_tuple(glm::vec3(pos), glm::vec3(force*0.01), glm::vec3(1,0,1)));
    
    // glm::normalize returns NaN for 0 vectors
    if (glm::length(segmentVel) < 0.01)
...
void AirfoilSegment::setFlaps(double flaps) {
    this->flaps = glm::clamp(flaps, 0.0, 1.0);
}
